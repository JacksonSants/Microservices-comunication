...
namespace ItemService.EventProcessor
{
    public class ProcessaEvento : IProcessaEvento
    {
        ...
        public void Processa(string mensagem)
        {
            ...
            var itemReponsitory = scope.ServiceProvider.GetRequiredService<IItemRepository>();
            ...
            if (itemReponsitory.ExisteRestauranteExterno(restaurante.Id))
            {
                itemReponsitory.CreateRestaurante(restaurante);
                itemReponsitory.SaveChanges();
            } 
        }
    }
}
